...
def posts():
    ...
    print(current_page)
    iCurrent_page = int(current_page)
    if 'username' in session:
        username = session['username']
    else:
        username = None
    ...
    # 게시물의 총 갯수
    query_count = "SELECT COUNT(*) as count FROM BOARD"
    # 마지막 페이지의 수 구하기
...
def login():
    if request.method == 'POST':
        ...
        conn = connectsql()
        cursor = conn.cursor()
        query = "SELECT * FROM tbl_user WHERE user_id = %s AND user_password = %s"
        value = (userid, userpw)
        cursor.execute(query, value)
